    ...
    def validate_time_slot(start_time: str, end_time: str) -> ValidationResult:
        ...
        try:
            ...
            if start_dt < work_start:
                warnings.append(
                    f"Start time {start_time} is before typical working hours ({Validator.WORK_DAY_START:02d}:00)"
                )
            ...
            for time_str, time_dt, label in [(start_time, start_dt, "Start"), (end_time, end_dt, "End")]:
                if time_dt.minute % Validator.TIME_RASTER != 0:
                    warnings.append(
                        f"{label} time {time_str} is not aligned with {Validator.TIME_RASTER}-minute raster"
                    )
    ...
    try:
        if input_type == "teacher_name":
            ...
            if start_widget and end_widget:
                return Validator.validate_time_slot(start_widget.currentText(), end_widget.currentText())

        # Add more UI validation types as needed

        return ValidationResult(is_valid=False, errors=[f"Unknown validation type: {input_type}"])
