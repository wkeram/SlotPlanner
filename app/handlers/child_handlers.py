...
from PySide6.QtWidgets import (
    QWidget, QPushButton, QLineEdit, QTableWidget, QComboBox, QCheckBox,
...
def child_delete_selected(window: QWidget, storage: Storage) -> None:
    ...
    def _delete_child():
        ...
        for tandem_name, tandem_data in data.get("tandems", {}).items():
            if (tandem_data.get("child1") == child_name or 
                tandem_data.get("child2") == child_name):
...
def child_dialog_add_availability_row(dialog: QWidget) -> None:
    ...
    combo_start.currentTextChanged.connect(
        lambda text: logger.debug(f"Start time changed to: {text}")
    )
    ...
    combo_end.currentTextChanged.connect(
        lambda text: logger.debug(f"End time changed to: {text}")
    )
...
def child_save_from_dialog(dialog: QWidget, window: QWidget, storage: Storage) -> None:
    ...
    if availability:
        ...
        if not availability_validation.is_valid:
            show_error(f"Child availability validation failed:\n\n{availability_validation.get_error_message()}", dialog)
            return
...
def _populate_child_edit_dialog(dialog: QWidget, child_name: str, child_data: dict, window: QWidget, storage: Storage) -> None:
    ...
    if name_field:
        ...
        name_field.setToolTip("⚠️ Changing the name will update all references to this child, including tandems and schedules.")
...
def _setup_child_edit_dialog(dialog: QWidget, window: QWidget, storage: Storage, original_child_name: str, original_child_data: dict) -> None:
...
def child_update_from_edit_dialog(dialog: QWidget, window: QWidget, storage: Storage, original_child_name: str) -> None:
    ...
    if name_changed:
        ...
        logger.info(f"Child rename: {original_child_name} -> {child_name}, {tandems_updated} tandems updated, {schedules_updated} schedule entries updated")
    ...
    if success:
        ...
        if hasattr(window, 'feedback_manager') and window.feedback_manager:
            if name_changed:
                window.feedback_manager.show_success(f"Child renamed from '{original_child_name}' to '{child_name}' successfully")
            else:
