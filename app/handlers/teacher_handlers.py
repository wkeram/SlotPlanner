...
def _populate_teacher_edit_dialog(dialog: QWidget, teacher_name: str, teacher_data: dict) -> None:
    ...
    if name_field:
        ...
        name_field.setToolTip("⚠️ Changing the name will update all references to this teacher, including children's preferences and schedules.")
...
def _setup_teacher_edit_dialog_buttons(dialog: QWidget, window: QWidget, storage: Storage, original_teacher_name: str) -> None:
...
def teacher_update_from_edit_dialog(dialog: QWidget, window: QWidget, storage: Storage, original_teacher_name: str) -> None:
    ...
    if name_changed:
        ...
        logger.info(f"Teacher rename: {original_teacher_name} -> {teacher_name}, {children_updated} children updated, {schedules_updated} schedule entries updated")
    ...
    if success:
        ...
        if hasattr(window, 'feedback_manager') and window.feedback_manager:
            if name_changed:
                window.feedback_manager.show_success(f"Teacher renamed from '{original_teacher_name}' to '{teacher_name}' successfully")
            else:
