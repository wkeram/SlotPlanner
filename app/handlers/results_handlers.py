...
from PySide6.QtWidgets import (
    QWidget, QTableWidget, QTableWidgetItem, QTextEdit, QComboBox, QProgressBar, QLabel
)
                    ...
                    for time_slot in time_slots:
                        if _is_valid_45min_slot(time_slot, time_slots):
                            objective_terms.append(
                                preferred_weight * assignments[(child, teacher, day, time_slot)]
                            )
                ...
                for day in days:
                    ...
                    for time_slot in time_slots:
                        if time_slot < "12:00" and _is_valid_45min_slot(time_slot, time_slots):
                            objective_terms.append(
                                early_weight * assignments[(child, teacher, day, time_slot)]
                            )
                ...
                for day in days:
                    for time_slot in time_slots:
                        if _is_valid_45min_slot(time_slot, time_slots):
                            ...
                            model.Add(
                                tandem_var <= assignments[(child1, teacher, day, time_slot)]
                            )
                            model.Add(
                                tandem_var <= assignments[(child2, teacher, day, time_slot)]
                            )
                            model.Add(
                                tandem_var >= assignments[(child1, teacher, day, time_slot)] +
                                              assignments[(child2, teacher, day, time_slot)] - 1
                            )
                            
                            objective_terms.append(
                                tandem_weight * priority * tandem_var
                            )
...
def generate_schedule_pdf(data, filename):
    ...
    if weights:
        weights_data = [
            ["Weight Parameter", "Value", "Description"]
        ]
