...
from PySide6.QtWidgets import (
    QWidget, QPushButton, QLineEdit, QTableWidget, QComboBox, QSpinBox, QMessageBox
)
...
def tandem_open_add_dialog(window: QWidget, storage: Storage) -> None:
    ...
    def _open_dialog():
        ...
        add_tandem_dialog.setWindowTitle("Add Tandem")
...
def tandem_edit_selected(window: QWidget, storage: Storage) -> None:
    ...
    def _edit_tandem():
        ...
        if not table:
            show_error("Tandems table not found", window)
            return
            
        selected_row = table.currentRow()
        if selected_row < 0:
            show_error("Please select a tandem to edit", window)
            return
        
        # Get tandem name from first column
        name_item = table.item(selected_row, 0)
        if not name_item:
            show_error("Could not get tandem name from selection", window)
            return
        ...
        if not tandem_data:
            show_error(f"Tandem data not found for: {tandem_name}", window)
            return
...
def tandem_delete_selected(window: QWidget, storage: Storage) -> None:
    ...
    def _delete_tandem():
        ...
        selected_row = table.currentRow()
        if selected_row < 0:
            show_error("Please select a tandem to delete", window)
            return
        ...
        if tandem_name in data.get("tandems", {}):
            ...
            if success:
                ...
                BaseHandler.show_info(
                    window,
                    "Tandem Deleted",
                    f"Tandem '{tandem_name}' has been deleted successfully."
                )
            else:
                logger.error(f"Failed to delete tandem: {tandem_name}")
                show_error(f"Failed to delete tandem '{tandem_name}'", window)
        else:
            show_error(f"Tandem '{tandem_name}' not found in data", window)
...
def _setup_tandem_dialog(dialog: QWidget, window: QWidget, storage: Storage) -> None:
    ...
    if not button_save:
        show_error("Save button not found in add_tandem.ui", window)
        return
    if not button_cancel:
        show_error("Cancel button not found in add_tandem.ui", window)
        return
...
def tandem_save_from_dialog(dialog: QWidget, window: QWidget, storage: Storage) -> None:
    ...
    if not name_field:
        show_error("Tandem name field not found in dialog", dialog)
        return
    
    tandem_name = name_field.text().strip()
    if not tandem_name:
        show_error("Please enter a tandem name", dialog)
        return
    ...
    if not child1_combo or not child2_combo:
        show_error("Child selection dropdowns not found in dialog", dialog)
        return
    ...
    if not child1_name or not child2_name:
        show_error("Please select both children for the tandem", dialog)
        return
    ...
    overlap_analysis = _analyze_availability_overlap(
        child1_data.get("availability", {}),
        child2_data.get("availability", {}),
        child1_name,
        child2_name
    )
    ...
    for existing_name, existing_data in data.get("tandems", {}).items():
        if (existing_data.get("child1") in [child1_name, child2_name] or 
            existing_data.get("child2") in [child1_name, child2_name]):
    ...
    # Save tandem data
    data.setdefault("tandems", {})[tandem_name] = {
        "child1": child1_name,
        "child2": child2_name,
        "priority": priority
    }
    ...
    if success:
        ...
        if hasattr(window, 'feedback_manager') and window.feedback_manager:
            window.feedback_manager.show_success(f"Tandem '{tandem_name}' saved successfully")
    else:
        logger.error(f"Failed to save tandem: {tandem_name}")
        show_error("Failed to save tandem data", dialog)
    ...
    if overlaps:
        ...
        for overlap in overlaps:
            overlap_lines.append(
                f"{overlap['day']}: {overlap['start']}-{overlap['end']} "
                f"({int(overlap['duration_minutes'])} min)"
            )
        overlap_summary = "\n".join(overlap_lines)
...
def _open_tandem_edit_dialog(window: QWidget, storage: Storage, tandem_name: str, tandem_data: dict) -> None:
    ...
    edit_tandem_dialog.setWindowTitle(f"Edit Tandem: {tandem_name}")
...
def _populate_tandem_edit_dialog(dialog: QWidget, tandem_name: str, tandem_data: dict, window: QWidget, storage: Storage) -> None:
    ...
    if child1_combo:
        ...
        logger.debug(f"Child1 combo populated with {len(all_children_for_dropdown)} items, current: '{child1_combo.currentText()}'")
    else:
    ...
    if child2_combo:
        ...
        logger.debug(f"Child2 combo populated with {len(all_children_for_dropdown)} items, current: '{child2_combo.currentText()}'")
    else:
    ...
    logger.debug(f"Pre-populated tandem edit dialog: {child1_name} + {child2_name}, priority {tandem_data.get('priority', 5)}")


def _setup_tandem_edit_dialog(dialog: QWidget, window: QWidget, storage: Storage, original_tandem_name: str, original_tandem_data: dict) -> None:
...
def tandem_update_from_edit_dialog(dialog: QWidget, window: QWidget, storage: Storage, original_tandem_name: str) -> None:
    ...
    if not new_tandem_name:
        show_error("Tandem name cannot be empty", dialog)
        return
    ...
    if name_changed:
        if new_tandem_name in data.get("tandems", {}):
            show_error(f"A tandem named '{new_tandem_name}' already exists. Please choose a different name.", dialog)
            return
    ...
    tandem_validation = Validator.validate_tandem_pair(child1_name, child2_name, priority)
    if not tandem_validation.is_valid:
        show_error(f"Tandem validation failed:\n\n{tandem_validation.get_error_message()}", dialog)
        return
    ...
    if success:
        ...
        if hasattr(window, 'feedback_manager') and window.feedback_manager:
            if name_changed:
                window.feedback_manager.show_success(f"Tandem renamed from '{original_tandem_name}' to '{new_tandem_name}' successfully")
            else:
                window.feedback_manager.show_success(f"Tandem '{new_tandem_name}' updated successfully")
    else:
        logger.error(f"Failed to update tandem: {new_tandem_name}")
        show_error("Failed to update tandem data", dialog)