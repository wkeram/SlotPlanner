...
class TestTandemScheduling:
    ...
    def test_basic_tandem_assignment(self, temp_storage, tandem_test_data):
        ...
        if len(tandem_assignments) == 2:
            ...
            assert (
                child_a_assignment["teacher"] == child_b_assignment["teacher"]
            ), "Tandem children should have same teacher"

            # Should be same day
            assert child_a_assignment["day"] == child_b_assignment["day"], "Tandem children should be on same day"
    ...
    def test_tandem_with_limited_availability(self, temp_storage):
        ...
        if len(tandem_assignments) == 2:
            ...
            times.sort()
            assert times == ["09:00", "10:00"], f"Should assign during availability overlap, got times {times}"
        else:
    ...
    def test_impossible_tandem_assignment(self, temp_storage):
        ...
        assert (
            len(assigned_children.intersection({"Child A", "Child B"})) >= 1
